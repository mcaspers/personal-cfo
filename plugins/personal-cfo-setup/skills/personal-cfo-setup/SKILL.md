...
Once the folder ID is confirmed, integration is complete. Do not ask the joining user to connect Finances, run `financial-warehouse-sync`, create a folder, or complete the first-time setup questions. Explain any limitation if their Google Drive account cannot access the shared folder.
...
### 5. Confirm before account-specific analysis
...
### 6. Build the private financial record
