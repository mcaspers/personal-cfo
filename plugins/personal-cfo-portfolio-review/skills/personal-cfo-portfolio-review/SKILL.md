...
Use the resolved top-level folder as the financial-data handoff. Use the newest dated `Financial Warehouse Sync - YYYY-MM-DD` and the canonical `Financial Data Warehouse`; inspect `Sources`, `Accounts`, `Balance_Snapshots`, `Investment_Holdings`, `Investment_Transactions`, and `Sync_State`. Require current `snapshot_as_of` and source status before presenting allocation results. Preserve login-required or otherwise stale snapshots as incomplete coverage, not zero holdings.
